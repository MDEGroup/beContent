...
if (!isset($_REQUEST['token'])) {
	...
	print_r($GLOBALS['becontent']->files);
	exit;
}

if (file_exists("{$GLOBALS['config']['cache_folder']}/".md5($_SERVER['QUERY_STRING']).".jpg")) {
	
	Header("Location: {$GLOBALS['config']['cache_folder']}/".md5($_SERVER['QUERY_STRING']).".jpg");
	exit;
} 


$table = $GLOBALS['becontent']->files[$_REQUEST['token']]['table'];
$field = $GLOBALS['becontent']->files[$_REQUEST['token']]['field'];

$entity = $GLOBALS['database']->getEntityByName($table);
...
if ((mysql_num_rows($oid) == 0) or ($data['foto_size'] == 0)) {
...
if ($type == FILE2FOLDER) {
	...
	$filename = "{$GLOBALS['config']['upload_folder']}/{$data[$field."_reference"]}";
...
switch ($data["{$field}_type"]) {
	case "image/jpeg":
	case "image/gif":
		$fp = fopen("{$GLOBALS['config']['cache_folder']}/".md5($_SERVER['QUERY_STRING']).".jpg", "w");
		break;
}
...
if (isset($_REQUEST['thumb'])) {
	...
	imagejpeg($thumb);
	imagejpeg($thumb, "{$GLOBALS['config']['cache_folder']}/".md5($_SERVER['QUERY_STRING']).".jpg");
