...
function ajaxDecode(input) {

	var php = new PHP_Serializer(); // use new PHP_Serializer(true); to enable UTF8 compatibility
	return (php.unserialize(input));         
}

function ajaxEncode(input) {
	var php = new PHP_Serializer(); // use new PHP_Serializer(true); to enable UTF8 compatibility
	return php.serialize(input);
}

function formPrune2(form) {
	
	var my_result = new Array(form.length); 
	
	for(var i=0; i<form.length; i++) {
		if (form.elements[i].type == "checkbox") {
			if (form.elements[i].checked) {
				my_result[form.elements[i].name] = form.elements[i].value;
			} else {
				my_result[form.elements[i].name] = "";
			}
...
function formPrune() {
	...
	var argv = formPrune.arguments;
  	var argc = argv.length;
  	
	var my_result = new Array(0); 
	
	for(j=0; j<argc; j++) {
		
		
		for(i=0; i<argv[j].length; i++) {
			
			my_result.length++;
			
			if (argv[j].elements[i].type == "checkbox") {
				
				if (argv[j].elements[i].checked) {
					
					my_result[argv[j].elements[i].name] = argv[j].elements[i].value;
				} else {
					
					my_result[argv[j].elements[i].name] = "";
				}
			 
			} else {
				if (argv[j].elements[i].type == "radio") {
					
					var radioName = eval('argv[j].elements[i].name');
					var radio = eval("argv[j]."+radioName);
